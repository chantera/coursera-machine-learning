...
for i = 1:m
  expect = zeros(num_labels, 1);
  delta3 = zeros(num_labels, 1);
  expect(y(i)) = 1
  for k = 1:num_labels
    J = J + (-expect(k) .* log(h(i,k)) - (1 - expect(k)) * log(1 - h(i,k)));
    delta3(k) = a3(i,k) - expect(k);
  end
  delta2 = (Theta2(:, 2:end)' * delta3) .* z2_g(:,i);
